...
export const VideoPlayer = ({ videoUrl, title }: VideoPlayerProps) => {
  ...
  useEffect(() => {
    if (videoUrl && (videoUrl.startsWith('http://') || videoUrl.includes('clouddn.com'))) {
  ...
  return (
    <div ref={containerRef} className="relative w-full bg-black rounded-xl overflow-hidden" onClick={showExternalLink ? undefined : handlePlayPause}>
      {showExternalLink ? (
        <div className="aspect-video flex flex-col items-center justify-center bg-gradient-to-br from-tech-blue/20 to-tech-purple/20">
          <div className="text-center px-6">
            <a href={videoUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-r from-tech-blue to-tech-purple border-glow-blue hover:scale-110 transition-transform duration-300 mx-auto mb-4">
